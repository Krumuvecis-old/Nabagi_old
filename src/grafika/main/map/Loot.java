        ...
        for(int[] chunkXY={0,0}; chunkXY[0]<laukums.size(); chunkXY[0]++){
            for( ; chunkXY[1]<laukums.get(chunkXY[0]).size(); chunkXY[1]++){
                ...
                for(int i=0;i<lietasList.size();i++) { //zîmç lietas, kas izmçtâtas pa karti
                    ...
                    double x = x0 + merogs * (lietasList.get(i).x + KonstantesUniversal.mapChunkW * chunkXY[0]),
                            y = y0 + merogs * (lietasList.get(i).y + KonstantesUniversal.mapChunkW * chunkXY[1]),
                    ...
                    for(int j=1; j<KonstantesUniversal.defaultLietas.size(); j++){ //pârbauda kuram tipam atbilst
                        if(KonstantesUniversal.defaultLietas.get(j).nosaukums.equals(lietasList.get(i).nosaukums)) {
