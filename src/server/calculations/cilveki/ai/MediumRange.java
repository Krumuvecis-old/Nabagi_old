...
////        double cenaTirgo= CilvekuKonstantes.paikaPriceDefault;// temporary default  value
...
////                ArrayList<Cilveks> cilvekiList = CalculationsThread.laukums.get(chunkXY[0]).get(chunkXY[1]).cilvekiList;
////
...
//            if (CalculationsThread.komandasList.size()>1 && (cilveks.komanda==CalculationsThread.komandasList.get(0).nosaukums && r.nextDouble()<orderChance)) { //ja ir bez komandas, pievienojas citai (izvçlâs  random, bet ne 0)
//                cilveks.komanda=CalculationsThread.komandasList.get(r.nextInt(CalculationsThread.komandasList.size()-1)+1).nosaukums;
//            } else if (r.nextDouble()<anarchyChance && cilveks.komanda!=CalculationsThread.komandasList.get(0).nosaukums) { //iespçja, ka pametîs esoðo komandu (tikai, ja kâdâ ir komandâ)
//                cilveks.komanda=CalculationsThread.komandasList.get(0).nosaukums;
//            }
//        }
//    }
//
...
//
//        double cenaCilvekam= CilvekuKonstantes.cenaCilvekam,
//                mantojumsCilvekamZelts=CilvekuKonstantes.mantojumsCilvekamZelts,
//                mantojumsCilvekamPaika=CilvekuKonstantes.mantojumsCilvekamPaika;
//
...
////        paikaTuvakaDist=-1;
////
////        for(int i = 0; i< CalculationsThread.laukums.get(biedrs.chunkXY[0]).get(biedrs.chunkXY[1]).lietas.size(); i++){
////            double distance = Math.hypot(cilveks.xyz.x- CalculationsThread.laukums.get(biedrs.chunkXY[0]).get(biedrs.chunkXY[1]).lietas.get(i).x,
////                    cilveks.xyz.y- CalculationsThread.laukums.get(biedrs.chunkXY[0]).get(biedrs.chunkXY[1]).lietas.get(i).y);
////
////			/*double resnumsJ; //nosaka lietas resnumu
////			if (CalculationsThread.lietas.get(j).nosaukums=="Zelts") { resnumsJ = zeltaResnums;
////			} else if (CalculationsThread.lietas.get(j).nosaukums=="Paika") { resnumsJ = paikasResnums;
////			} else resnumsJ = lietasResnums;*/
////
////            if(CalculationsThread.laukums.get(biedrs.chunkXY[0]).get(biedrs.chunkXY[1]).lietas.get(i).nosaukums=="Zelts") {
...
////            }
////
////            if(CalculationsThread.laukums.get(biedrs.chunkXY[0]).get(biedrs.chunkXY[1]).lietas.get(i).nosaukums=="Paika") {
